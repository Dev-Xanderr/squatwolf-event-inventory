...
function SendBackBanner({
  ...
  useEffect(() => {
    ...
    let q = sb.from('event_comments').select('*').eq('event_id', event.id).like('body', '↶ Sent back:%');
...
function WorkflowTracker({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    const meta = status === 'done' ? enteredAt[s.v] || (event.workflow_updated_by ? {
      by: event.workflow_updated_by,
      at: event.workflow_updated_at
    } : null) : null;
...
function App() {
  ...
  useEffect(() => {
    if (!admin) return;
    const ch = sb.channel(`notif-workflow-${admin.id || admin.name}`).on('postgres_changes', {
      event: 'INSERT',
      schema: 'public',
      table: 'history'
    }, async ({
      ...
      const {
        data: ev
      } = await sb.from('events').select('name').eq('id', row.event_id).maybeSingle();
      if (!ev) return;
